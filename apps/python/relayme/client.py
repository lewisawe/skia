...
and return a text-first, fail-closed structured result.

Mock mode (default) runs with no credentials and no network: it prints the call
preview and replays a fixture through the same result-normalisation code the live
path uses. The --execute path drives the real `calle` CLI:
...
The live result is read from the CLI envelope at result.structuredContent (plan)
...
def normalize_result(raw: dict) -> dict:
    """Coerce any result into the fail-closed RelayMe schema.

    Unknown or missing outcomes fail closed to needs_human. We never invent an
    answer we did not receive.
    """
    ...
    if outcome not in {"answered", "partial"}:
        # Only answered/partial may carry a user-facing answer.
        answer = answer if outcome == "partial" else answer
    return {
        "answer": answer if outcome in {"answered", "partial"} else "",
        "outcome": outcome,
...
def print_preview(task: dict, goal: str) -> None:
    ...
    for line in goal.splitlines():
        print(f"    {line}")
    print("\n  Result schema (text-first, fail-closed):")
    print("    answer, outcome, transcript_summary, follow_up_needed, disclosed_ai")
    print("=" * 68)


def run_mock(task: dict, fixture_path: Path) -> dict:
    fixture = json.loads(fixture_path.read_text())
    print("\n[mock] Replaying fixture transcript (no live call):\n")
    for turn in fixture.get("mock_transcript", []):
        who = "Agent " if turn["speaker"] == "agent" else "Callee"
        print(f"    {who}: {turn['text']}")
    return normalize_result(fixture.get("structured_result", {}))


def _calle(argv: list[str]) -> dict:
    """Run a `calle` CLI command and return parsed JSON stdout."""
    proc = subprocess.run(
        ["calle", *argv], capture_output=True, text=True, check=False
    )
...
def run_live(task: dict, goal: str, poll_seconds: int = 10, max_polls: int = 30) -> dict:
    """Drive the real CALL-E plan -> run -> status flow via the calle CLI."""
    ...
    if not plan.get("plan_id") or plan.get("ready_to_run") is False:
        return normalize_result({"outcome": "needs_human",
    ...
    for _ in range(max_polls):
        ...
        if sc.get("state") in {"completed", "failed", "ended"} or sc.get("structured_result"):
            return normalize_result(sc.get("structured_result", sc))
...
def main() -> int:
    ...
    ap.add_argument("--mock", action="store_true", help="no-call preview + fixture replay (default if --execute absent)")
    ...
    if not args.execute:
        ...
        result = run_mock(task, fixture)
    else:
        print("\n[live] Placing a real call via CALL-E...")
        result = run_live(task, goal)

    print("\nStructured result:")
    print(json.dumps(result, indent=2))
    print(f"\nOutcome: {result['outcome']}")
    if result["outcome"] in {"answered", "partial"}:
        print(f"Answer for the user:\n  {result['answer']}")
    else:
        print("No confirmed answer. Routed for a human to review.")
    return 0
